...
//document.addEventListener("DOMContentLoaded", function(event) {
	...
	search_button.addEventListener('click', buscarSPCad)


	function buscarSPCad(e) {
		//if (search.value != "") {
		var myform = new FormData()
		myform.append("cadena", search.value)
		myform.append("filtroActual", filtroActual.value)

		fetch(base_url_js+'Seguimientos/buscarSPPorCadena', {
				method: 'POST',
				body: myform
			})
			...
			.then(function(myJson) {
				...
				if (!(typeof(myJson) == 'string')) {
					document.getElementById('id_header').innerHTML = myJson.infoTable.header
...
{	
	...
	fetch(base_url_js+'Seguimientos/setColumnFetchSP', {
			method: 'POST',
			body: myform
		})
	.then(function(response){
