...
search_button.addEventListener('click',buscarInteligenciaOperativaCad);

function buscarInteligenciaOperativaCad(e){
    let  myform = new FormData();
    myform.append('cadena', search.value);
    myform.append('filtroActual', filtroActual.value);

    fetch(base_url_js+'InteligenciaOperativa/buscarPorCadena',{
        method: 'POST',
        body: myform
    })
    .then(res=>res.json())
...
const hideShowColumn = (col_name)=>{
    ...
    fetch(base_url_js+'InteligenciaOperativa/setColumnFetch',{
        method: 'POST',
        body: myform
    })
    .then((res)=>{
...
const checarCadena = (e)=>{
    if ((search.value == "") || (e.keyCode == '13')) {
        buscarInteligenciaOperativaCad()
    }
}